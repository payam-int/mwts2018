 ...
 */
class Version20180302132408 extends AbstractMigration
{
    public function up(Schema $schema)
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__payment AS SELECT id, user_id, reference_id, price, state, metadata, online_payment_price, done, creation_date, payment_done_date FROM payment');
        ...
        , online_payment_price VARCHAR(255) NOT NULL COLLATE BINARY, done BOOLEAN NOT NULL, creation_date DATETIME NOT NULL, payment_done_date DATETIME NOT NULL, PRIMARY KEY(id), CONSTRAINT FK_6D28840DA76ED395 FOREIGN KEY (user_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO payment (id, user_id, reference_id, price, state, metadata, online_payment_price, done, creation_date, payment_done_date) SELECT id, user_id, reference_id, price, state, metadata, online_payment_price, done, creation_date, payment_done_date FROM __temp__payment');
    ...
    public function down(Schema $schema)
    {
        ...
        $this->addSql('CREATE INDEX IDX_E1E0B40E4C3A3BB ON discount (payment_id)');
        $this->addSql('DROP INDEX IDX_6D28840DA76ED395');
        $this->addSql('CREATE TEMPORARY TABLE __temp__payment AS SELECT id, user_id, reference_id, price, online_payment_price, state, metadata, done, creation_date, payment_done_date FROM payment');
        $this->addSql('DROP TABLE payment');
        ...
        , done BOOLEAN NOT NULL, creation_date DATETIME DEFAULT \'0\' NOT NULL, payment_done_date DATETIME DEFAULT \'0\' NOT NULL, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO payment (id, user_id, reference_id, price, online_payment_price, state, metadata, done, creation_date, payment_done_date) SELECT id, user_id, reference_id, price, online_payment_price, state, metadata, done, creation_date, payment_done_date FROM __temp__payment');
        $this->addSql('DROP TABLE __temp__payment');
        $this->addSql('CREATE INDEX IDX_6D28840DA76ED395 ON payment (user_id)');
        $this->addSql('DROP INDEX IDX_7DAAE07DA76ED395');
        $this->addSql('DROP INDEX UNIQ_7DAAE07D7294869C');
